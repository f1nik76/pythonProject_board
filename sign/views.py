...
def verify_view(request):
    ...
    if pk:
        ...
        try:
            ...
            messages.error(request, 'Устаревший код')
            return redirect('login')

        if not request.POST:
            email = user.email
            message_body = f'Уважаемый {user.username}, одноразовый пароль {user_code}. Используйте этот пароль для проверки.'
            send_mail(message=message_body, from_email='f1nik.yar@yandex.ru', subject='Одноразовый код', recipient_list=[email])
        if form.is_valid():
